...
def anomalyDetection_example():
    '''加载并显示数据'''
    data = spio.loadmat(BASE_DIR / 'data1.mat')
    X = data['X']
...
def visualizeFit(X,mu,sigma2):
    ...
    if np.sum(np.isinf(Z).astype(float)) == 0:   # 如果计算的为无穷，就不用画了
        #plt.contourf(X1,X2,Z,10.**np.arange(-20, 0, 3),linewidths=.5)
        CS = plt.contour(X1, X2, Z, 10.**np.arange(-20, 0, 3), colors='black', linewidths=.5)
        #plt.clabel(CS)
