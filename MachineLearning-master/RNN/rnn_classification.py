...
RNN 是最基础的循环网络，每一步把当前输入和上一步隐状态拼接后更新隐状态，
适合处理序列数据。但长序列容易出现梯度消失。

本脚本用合成数据演示 RNN 的时序二分类：
    - 类别 0：正弦波
    - 类别 1：余弦波
每条序列长度 50，带随机相位偏移。

依赖：PyTorch、numpy、scikit-learn
运行：python rnn_classification.py
"""
import numpy as np
import torch
...
class SimpleRNN(nn.Module):
    ...
    def __init__(self, input_size=1, hidden_size=32, num_layers=1):
        super().__init__()
        # RNN 层：input_size 个特征每步输入，hidden_size 个隐状态
        self.rnn = nn.RNN(input_size, hidden_size, num_layers, batch_first=True)
        # 全连接层：把最后的隐状态映射到 2 类
        self.fc = nn.Linear(hidden_size, 2)

    def forward(self, x):
        """前向传播：RNN 输出后取最后一个时间步的隐状态做分类。"""
...
def main():
    ...
    print("RNN test acc:", accuracy_score(yte, pred))
